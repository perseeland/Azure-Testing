...
/* ═══════════════════════════════════════════════════════════
   SIDEBAR — COLLAPSE ALL NAV GROUPS
═══════════════════════════════════════════════════════════ */

function buildNavCollapseAllButton() {
    const sideNav = document.querySelector('.side-nav');
    const h2 = sideNav && sideNav.querySelector('h2');
    const groups = sideNav && sideNav.querySelectorAll('[data-navgroup]');
    if (!sideNav || !h2 || !groups || groups.length === 0) return;

    const btn = document.createElement('button');
    btn.className = 'nav-collapse-all-btn';
    btn.setAttribute('data-collapsed', 'false');
    btn.innerHTML = '⊖ Collapse All';

    btn.addEventListener('click', () => {
        const isCollapsed = btn.getAttribute('data-collapsed') === 'true';
        sideNav.querySelectorAll('[data-navgroup]').forEach(item => {
            const groupId = item.getAttribute('data-navgroup');
            const body  = document.querySelector(`[data-navbody="${groupId}"]`);
...
/* ═══════════════════════════════════════════════════════════
   CLEAR INPUTS — clears the text inputs inside the input bar
   that the button belongs to, then re-renders the commands.
═══════════════════════════════════════════════════════════ */
function clearInputs(btn) {
    const bar = btn.closest('.var-bar, .user-bar');
    if (bar) bar.querySelectorAll('input').forEach(el => { el.value = ''; });
    if (typeof azUpdate === 'function') azUpdate();
}
